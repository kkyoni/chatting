...
class Helper
{
    ...
    public static function addToLog($subject, $status, $user_id, $table_name, $table_id,$response)
    {
        $log = [];
        $log['subject'] = $subject;
        ...
        $log['status'] = $status;
        $log['table_name'] = (!empty($table_name)) ? $table_name : '';
        $log['table_id'] = (!empty($table_id)) ? $table_id : 0;
        $log['user_id'] = (!empty($user_id)) ? $user_id : 0;
        $log['response'] = (!empty($response)) ? $response : null;
